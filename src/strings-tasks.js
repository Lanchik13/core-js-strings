...
function formatTime(minutes, seconds) {
  const minutesRez = Math.max(0, Math.floor(minutes));
  const secondsRez = Math.max(0, Math.floor(seconds));

  return `${minutesRez.toString().padStart(2, '0')}:${secondsRez
    .toString()
    .padStart(2, '0')}`;
}
...
function isPalindrome(str) {
  const strSplit = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return strSplit === strSplit.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const removePunctuation = (word) => word.replace(/[^a-zA-Z]/g, '');

  const longestWord = wordsArr.reduce((longest, current) => {
    const cleanedWord = removePunctuation(current);
...
function invertCase(str) {
  const invertedChars = Array.from(str, (char) => {
...
function extractNameFromTemplate(value) {
  const parts = value.split('Hello, ').join('').split('!').join('').trim();
  return parts;
}
